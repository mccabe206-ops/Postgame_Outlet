...
focused tests are complete. A fresh actual Fable re-review attempt at **19:28
EDT on September 8, 2026** again returned HTTP 429, now reporting a Fable-specific
usage limit with **no reset time**. It consumed zero tokens and produced no
review verdict. The earlier 15:40 reset is not a current readiness estimate.
The [new failure receipt](reviews/fable-rereview-service-failure-20260908T232855Z.json)
preserves the attempt separately from the earlier failure. Parent retains the
no-fit gate; independent local checks do not substitute for that review. Resume
only after the actual re-review and an explicit parent fit go-ahead. The user
authorized evaluation, not promotion.
...
Resume result: **15 passed** (the original 14 plus one independent-verifier
regression check). These include hand-calculated descriptors, strict DOB and
...
The September 8 evening resume also rechecked all six historical and four
current preparation members, 23 protected code hashes and 67 raw source hashes.
All matched; the current diagnostic retains its original input clock. The
[resume receipt](resume-20260908T232831Z.json) records these checks, the checker
review, and unchanged hashes for 156 public/evidence/data/workflow files.
...
The independent saved-result checker is now available in [verify.py](verify.py).
It reuses the earlier independent arithmetic helpers rather than the trainer's
prediction or metric functions. Its no-fit regression check covers the three
decision criteria, corrupt saved coefficients, nonfinite arithmetic and output
overwrite refusal. The checker has **not** been run against real candidate
results because the candidate has not been fitted.

After the single fit is reviewed, authorized and completed, run:

```powershell
python -B -m research.pgo_corrected_roster_candidate.verify --run research/pgo_corrected_roster_candidate/run-20260908 --output research/pgo_corrected_roster_candidate/saved-fit-verification.json
```

The verification output must not already exist. It checks saved fits and
predictions, cohort and source preservation, training-only preprocessing,
metrics/bootstrap, the fixed three-part screen, and signed/neutral symmetry.
It cannot supply Fable's verdict or authorize fitting, issuance or adoption.

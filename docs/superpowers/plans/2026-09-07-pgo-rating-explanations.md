...
- [x] Keep the visible primer short and the table easy to reach. Put all four rank-highlight lists in a native disclosure and the existing technical metadata/backtest paragraph in a separate idempotent disclosure. Retain original refreshable metadata text inside. New team controls use the existing `row-trigger` style, and injected headings retain `id="drawerTitle"` for the shared dialog's accessible name. Displayed rounded components must sum to their displayed total.
